<template>
	<div class="editor">
      ...
      >添加任务</el-button
    > -->
		<div v-show="true" class="editor-wrapper">
			<QuillEditor
				ref="QuillEditorRef"
		...
		<el-button-group v-show="isEdit" class="button-group">
...
const addQuillToolBarTitle = () => {
	...
	aButtons.forEach((button: HTMLButtonElement) => {
		if (button.className === 'ql-script') {
			button.title = button.value === 'sub' ? '下标' : '上标'
		} else if (button.className === 'ql-indent') {
			button.title = button.value === '+1' ? '向右缩进' : '向左缩进'
		} else {
...
const handleTextChange = (/** {
  ...
  source
	...
	const textContent = QuillEditorRef.value?.getText()
	if (textContent?.trim()) {
